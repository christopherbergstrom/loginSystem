...
if (isset($_POST['login-submit'])) {
  ...
  // changed these to my post variable names
  ...
  if (empty($mailuid) || empty($password)) {
    // again changed this to my username variable
    header("Location: ../index.php?error=emptyfields&username=".$mailuid);
    exit();
  }
  else {
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
      ...
      if ($row = mysqli_fetch_assoc($result)) {
        ...
        // Then if they DO match, then we know it is the correct user that is trying to log in!
          ...
          // changed these to match my DB column names
          $_SESSION['id'] = $row['id'];
